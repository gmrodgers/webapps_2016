...
class DataServer: NSObject, NSURLConnectionDelegate {
  ...
  func createNewCard(card: Card, controller: CardTableViewController) {
    ...
    let postData = ["card" : ["topic_id" : String(card.topic_id), "cardname" : card.name, "card_data" : card.htmlData, "image_loc" : card.imageRef, "question" : card.question, "answer" : card.answer]]
    let httpMethod = "POST"
